          ...
          <div className="h-64 bg-gradient-to-br from-orange-400 to-orange-600 relative">
            ...
            {isAdmin && (
              <button
                ...
                className="absolute top-6 right-6 px-4 py-2 bg-white/90 backdrop-blur-sm rounded-xl text-gray-900 font-semibold hover:bg-white transition-all duration-200 shadow-md flex items-center gap-2 relative z-20"
              >
                <Settings className="w-4 h-4" />
                Настройки
                {/* Notification Badge for pending change requests */}
                {pendingChangeRequestsCount > 0 && (
                  <span className="absolute -top-2 -right-2 min-w-[24px] h-6 bg-red-500 text-white text-xs font-bold rounded-full flex items-center justify-center px-2 animate-pulse">
